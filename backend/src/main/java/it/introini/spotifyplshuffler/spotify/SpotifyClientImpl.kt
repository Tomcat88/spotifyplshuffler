...
class SpotifyClientImpl @Inject constructor(private val config: Config,
                                            vertx: Vertx) : SpotifyClient {
    ...
    override fun getAuthorizeUrl(state: String): String {
    ...
    override fun requestTokens(code: String, future: Future<JsonObject>) {
    ...
    override fun refreshToken(token: Token): Pair<Throwable?, JsonObject?> {
        val auth = getClientAuthorizationHeader()
        val encoder = QueryStringEncoder("")
        encoder.addParam("grant_type", "authorization_code")
        encoder.addParam("redirect_uri", redirectUri)
        ...
        result.let { (data, error) ->
            return when {
                ...
                data != null -> Pair(null, data.let { JsonObject(it) })
                else -> Pair(IllegalStateException("data and error both null"), null)
            }
        }
    }

    override fun getMe(token: Token, future: Future<SpotifyUser>) {
    ...
    override fun getPlaylist(token: Token, uid: String, pid: String): Future<PlaylistFull> {
    ...
    override fun getPlaylists(token: Token, future: Future<PagingObject<SpotifyPlaylist>>) {
    ...
    override fun getPlaylistTracks(token: Token, uid: String, playlist: String, future: Future<Collection<SpotifyPlaylistTrack>>) {
    ...
    override fun createPlaylist(token: Token, uid: String, name: String, public: Boolean, collaborative: Boolean, description: String): Future<SpotifyPlaylistFull> {
    ...
    override fun addTracks(token: Token, uid: String, pid: String, trackIds: Collection<String>): Future<JsonArray> {
    ...
    override fun getDevices(token: Token): Future<SpotifyDevices> {
    ...
    override fun getCurrentPlayback(token: Token): Future<SpotifyCurrentPlayingContext> {
    ...
    override fun startPlayback(token: Token, deviceId: String?): Future<Void> {
        return controlPlayback(START_PLAYBACK, token, deviceId)
    }

    override fun stopPlayback(token: Token, deviceId: String?): Future<Void> {
        return controlPlayback(PAUSE_PLAYBACK, token, deviceId)
    }

    override fun skipToNextPlayback(token: Token, deviceId: String?): Future<Void> {
        return skipPlayback(NEXT_PLAYBACK, token, deviceId)
    }
    ...
    private fun skipPlayback(url: String, token: Token, deviceId: String?): Future<Void> {
        val deviceString = deviceId?.let { "?device_id=$it" } ?: ""
        return postRequest<Void>("$url$deviceString", token,JsonObject()).let {
            (error, _) -> if (error != null) Future.failedFuture(error) else Future.succeededFuture()
        }
    }
    ...
    inline private fun <reified T> postRequest(url: String, token: Token, dataObject: JsonObject, typeReference: TypeReference<*>? = null): Pair<Throwable?, T?> {
        ...
        result.let { (data, error) ->
            when {
                ...
                data != null -> return try {
                    when {
                        T::class == Void::class -> { Logger.info("void result"); Pair(null, null) }
                        typeReference == null -> Pair(null, data.let { JsonObject(it) }.let {  if (T::class != JsonObject::class) it.mapTo(T::class.java) else T::class.java.cast(it) })
                        else -> Pair(null, Json.mapper.convertValue(data.let { JsonObject(it).map }, typeReference))
                    }
                } catch (t: Throwable) {
                    Logger.error(t, "Unknown exception")
                    Pair(t, null)
